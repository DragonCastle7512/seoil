...
레이아웃 배치 방식
태그의 구성요소
기초 스타일 속성 다루기(width, height, margin, padding 등)
display 속성(inline, block, flex 등)
개발자 도구 사용법(f12)
레이아웃 배치 연습(NAVER)
```
### 5/9
```
웹의 기본 동작 원리
DB Connection에 필요한 3가지
DB 연결 과정
html 레이아웃(header, nav, footer 등)
웹 크기 고정값과 가변값 처리
레이아웃 배치 연습
```
### 5/12
...
### 6/3
```
h2 database -> oracle로 바뀌었을 때 코드 수정
